...
output_dir = '/scratch/kvg245/vidsal_gan/vidsal_gan/output/futIRCCYN_30_l1_100_1/'
target_file = 'maps_data_saliency.h5'
input_file = 'maps_data.h5'
index_file = 'fut_indices'
...
#	progress  = ProgressBar(len(data_list),fmt=ProgressBar.FULL)
	...
	for data in data_list:
	    ...
	    frames = np.asarray([video[x] for x in data[1:num_frames+1]])
	    maps = np.asarray([target[x] for x in data[num_frames+1:]])
...
#	progress.done()
	...
	dummy_2 = np.zeros((target_batch.shape[0],target_batch.shape[2],target_batch.shape[3],target_batch.shape[1]))
	dummy[:,:,:,:3] = input_batch[:,0,:,:,:]
	dummy[:,:,:,3:6] = input_batch[:,1,:,:,:]
	dummy[:,:,:,6:9] = input_batch[:,2,:,:,:]
	dummy[:,:,:,9:] = input_batch[:,3,:,:,:]
	dummy_2[:,:,:,0] = target_batch[:,0,:,:]
        dummy_2[:,:,:,1] = target_batch[:,1,:,:]
        dummy_2[:,:,:,2] = target_batch[:,2,:,:]
        dummy_2[:,:,:,3] = target_batch[:,3,:,:]

	
	return {'input':dummy,'target':dummy_2}
...
def main():
    ...
    target = tf.placeholder(dtype = tf.float32, shape = (batch_size,256,256,num_fut))
